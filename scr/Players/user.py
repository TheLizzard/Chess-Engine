...
class User(Player):
    ...
    def create_user_helping_object(self, start, end):
        if start == end:
            _hash = hash(start*2)
    ...
    def delete_user_created_object(self, idx=None):
        if idx is None:
    ...
    def stop_user_created_object(self):
        if self.user_helper_making is not None:
    ...
    def position_to_piece(self, position):
